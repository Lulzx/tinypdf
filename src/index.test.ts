...
describe('markdown images', () => {
  ...
  test('renders image from images option', () => {
    const bytes = markdown('![Logo](logo.jpg)', {
      images: { 'logo.jpg': minimalJpeg }
    })
    const str = new TextDecoder().decode(bytes)
    expect(str).toContain('/Type /XObject')
    expect(str).toContain('/Subtype /Image')
  ...
  test('skips image if not in images map', () => {
    const bytes = markdown('![Logo](missing.jpg)', {
      images: {}
    })
    const str = new TextDecoder().decode(bytes)
  ...
  test('renders text before and after image', () => {
    const bytes = markdown(`# Title

![Photo](photo.jpg)

Some text after.`, {
      images: { 'photo.jpg': minimalJpeg }
    })
    const str = new TextDecoder().decode(bytes)
    expect(str).toContain('(Title) Tj')
    expect(str).toContain('(Some text after.) Tj')
    expect(str).toContain('/Type /XObject')
  })

  test('handles multiple images', () => {
    const bytes = markdown(`![First](a.jpg)
...
![Second](b.jpg)`, {
    ...
    const str = new TextDecoder().decode(bytes)
    // Should have two image XObjects
    const xobjectMatches = str.match(/\/Type \/XObject/g)
    expect(xobjectMatches?.length).toBe(2)
  })

  test('parses image with alt text', () => {
    const bytes = markdown('![My Alt Text](image.jpg)', {
      images: { 'image.jpg': minimalJpeg }
    })
    const str = new TextDecoder().decode(bytes)
    expect(str).toContain('/Type /XObject')
  })
  ...
  test('parses image with empty alt text', () => {
    const bytes = markdown('![](image.jpg)', {
      images: { 'image.jpg': minimalJpeg }
    })
    const str = new TextDecoder().decode(bytes)
    expect(str).toContain('/Type /XObject')
  ...
  test('works without images option', () => {
    const bytes = markdown('# Hello\n\n![img](test.jpg)')
    expect(bytes).toBeInstanceOf(Uint8Array)
    expect(bytes.length).toBeGreaterThan(0)
  })

  test('image uses Do operator', () => {
    const bytes = markdown('![Logo](logo.jpg)', {
      images: { 'logo.jpg': minimalJpeg }
    })
    const str = new TextDecoder().decode(bytes)
